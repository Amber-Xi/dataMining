...
def pca_with_covar_matrix(D, target_var):
    ...
    '''
    print_vector = []
    print_vector.append(eig_vecs[0])
    print_vector.append(eig_vecs[1])
    '''
    C_r = eig_vecs[:,:2]
    print(np.shape(C_r))
    return np.matmul(Z, C_r)

    #return np.matmul(Z, np.transpose(print_vector))
...
with open("energydata_complete.csv") as f:
    ...
    D = pd.read_csv("energydata_complete.csv", delimiter=',', skiprows=1,nrows = 1000,usecols=range(1,ncols-1),header = None)
    D = D.to_numpy()

    #Perform kernel PCA using the linear kernel, plot data
    A = _kernel_pca(D, linear_kernel, 0.95)
    scatter_plot(A, "Linear Kernel PCA")
    #C = _kernel_pca(D, gaussian_kernel, 50)
    #scatter_plot(C, "Gaussian Kernel PCA")
    
    B = pca_with_covar_matrix(D, 0.95)
