...
with open("energydata_complete.csv") as f:
    ncols = len(f.readline().split(','))

data = pd.read_csv("energydata_complete.csv", delimiter=',', skiprows=1, usecols=range(1,ncols-1),header = None)
data = data.to_numpy()

#computing the mean
column_mean = np.sum(data, axis=0)/data.shape[0]
...
ev = powerIteration(cov_inner, 0.0001)
...
# =============================================================================
for row in data:
    projections.append((row @ ev_unit))
print(projections)
plt.figure(figsize=(5,5))
...
def power_iteration(A):
    ...
    while True:
        ...
        print("Itr: {:} ; Eigen Val: {:.3f}; Eigen Vec {:}; error{:.2f}".format(k,ev_new,v_new,err))
...
e = power_iteration(cov_inner)
proj1 = np.dot(Z_matrix, e)
print("- Projection and new coordinates")
print(proj1)
'''
